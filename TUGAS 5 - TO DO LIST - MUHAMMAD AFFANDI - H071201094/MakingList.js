...
btnAdd.addEventListener("click", () => {
    ...
    if (database.has(ITEM_KEY)) {
        const duplicateConfirm = confirm('Kamu Sudah Memiliki Barang Yang Sama Yaitu "' + ITEM_VALUE + '". Apakah Kamu Ingin Menambahkannya Lagi ?');
    ...
    btnDelete.addEventListener('click', () => {
        const delConfirm = confirm("Apakah Kamu Ingin Menghapus Barang Tersebut ?")
        if (delConfirm) {
            const getCounter = document.getElementById(ITEM_KEY);
